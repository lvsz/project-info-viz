document.addEventListener('DOMContentLoaded', function(event) {
  ...
  const macRawCSV = 'mac-raw.csv';  //'big-mac-raw-index.csv';
  const macAdjCSV = 'mac-adj.csv';  //'big-mac-adjusted-index.csv';
  const getCpiCSV = (country) => `CPI_${country}.csv`;
  ...
  Promise
      .all([
        d3.csv(macRawCSV), d3.csv(getCpiCSV(chosenCPI)),
        d3.csv(
  ...
  function changeData() {
    ...
    Promise
        .all([d3.csv(
            'https://raw.githubusercontent.com/lvsz/project-info-viz/node-server/data/rateinf/CPI_' +
            chosenCPI + '.csv')])
        .then(function(values) {
          // cpi
          CPI = values[0]
